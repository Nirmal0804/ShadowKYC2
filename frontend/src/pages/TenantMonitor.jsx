...
const TenantMonitor = () => {
    ...
    useEffect(() => { clientConnectedRef.current = clientConnected; }, [clientConnected]);
    const [codeCopied, setCodeCopied] = useState(false);
    const [wsError, setWsError] = useState(null); // null | 'not_found' | 'auth'
    ...
    const connectTenantWS = async () => {
        if (!sessionId) return;
        const token = await getAccessToken();
        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const wsUrl = `${protocol}//${window.location.host}/ws/session/${sessionId}/tenant?v=${Date.now()}&token=${token}`;
        ...
        socket.onclose = (e) => {
            ...
            // 4001: Auth failed (should rarely happen now with guest fallback on backend)
            if (e.code === 4001) {
                setClientConnected(false);
                setWsError('auth');
                return;
            }
            // 4004: Session not found — likely server restarted and lost in-memory sessions
            if (e.code === 4004) {
                setClientConnected(false);
                setWsError('not_found');
                return;
            }
    ...
    // --- Graceful error screen when session is not found or auth fails ---
    if (wsError) {
        const isNotFound = wsError === 'not_found';
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    borderRadius: '1.5rem', padding: '3rem 2.5rem', textAlign: 'center',
                    ...
                    <div style={{
                        ...
                        background: isNotFound ? 'rgba(245,158,11,0.15)' : 'rgba(239,68,68,0.15)',
                        border: `1px solid ${isNotFound ? 'rgba(245,158,11,0.4)' : 'rgba(239,68,68,0.4)'}`,
                    ...
                    <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '0.75rem' }}>
                        {isNotFound ? 'Session Expired' : 'Authentication Error'}
                    </h2>
                    <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '2rem' }}>
                        {isNotFound
                            ? 'This session no longer exists. The server may have restarted and cleared active sessions. Please create a new session from the dashboard.'
                            : 'Your login session has expired. Please log in again to continue.'}
                    </p>
                    <button
                        onClick={() => navigate(isNotFound ? '/home' : '/login')}
                        style={{
                    ...
                    >
                        {isNotFound ? 'Create New Session' : 'Return to Login'}
                    </button>
                </div>
            </div>
        );
