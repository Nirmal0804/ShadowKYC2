...
import { Shield, User, LogOut, Plus, ArrowRight, Monitor, Upload, FileVideo, CheckCircle, AlertTriangle, X, Loader, Sun, Moon, Mail, Building, Hash, Calendar, Bell, Clock, MessageSquare, FileText, History, BarChart3, HelpCircle, Settings, ChevronRight, Send, Eye, LayoutDashboard, BarChart2, Video, ShieldCheck, Users, AlertCircle, Search, MoreVertical, PieChart, Activity, UserCheck, ShieldAlert } from 'lucide-react';
...
const Home = () => {
    ...
    const fetchNotifications = async (uid) => {
        try { const r = await fetchWithAuth(`/notifications/${uid}`); const d = await r.json(); setNotifications(d.notifications || []); setUnreadCount((d.notifications || []).filter(n => !n.read).length); } catch (e) { }
    };
    const fetchHistory = async (uid) => {
        try { const r = await fetchWithAuth(`/session/history/${uid}?role=${role}`); const d = await r.json(); setSessionHistory(d.history || []); } catch (e) { }
    };
    const fetchAppStatus = async (uid) => {
        try { const r = await fetchWithAuth(`/application/status/${uid}`); const d = await r.json(); setAppStatuses(d.statuses || []); } catch (e) { }
    };
    const fetchTickets = async (uid) => {
        try { const r = await fetchWithAuth(`/support/tickets/${uid}`); const d = await r.json(); setTickets(d.tickets || []); } catch (e) { }
    };
    const fetchTenantStats = async (uid) => {
        try { const r = await fetchWithAuth(`/tenant/stats/${uid}`); const d = await r.json(); setTenantStats(d); } catch (e) { }
    };
    const fetchDocuments = async () => {
        try { const r = await fetchWithAuth(`/tenant/documents`); const d = await r.json(); setDocuments(d.documents || []); } catch (e) { }
    };
    const fetchTenantTickets = async () => {
        try { const r = await fetchWithAuth(`/tenant/tickets`); const d = await r.json(); setTenantTickets(d.tickets || []); } catch (e) { }
    };
    ...
    const markNotificationsRead = async () => {
        try { await fetchWithAuth(`/notifications/${user.id}/read`, { method: 'POST' }); setUnreadCount(0); setNotifications(prev => prev.map(n => ({ ...n, read: true }))); } catch (e) { }
    };
    ...
    const handleCreateSession = async () => {
        ...
        setSessionCode('');
        try {
            // Use the shadow_token stored at login (works for both Supabase & custom auth)
            const token = localStorage.getItem('shadow_token') || '';
            if (!token) {
                setCreateError('You are not logged in. Please log in and try again.');
                return;
            }
            ...
            if (response.status === 401) {
                setCreateError('Invalid or expired session. If you just logged in, please wait a moment and try again.');
                return;
            }
            ...
            if (response.ok) {
                ...
                setCreateError(data.detail || 'Failed to create session. Please try again.');
                console.error("Failed to create session:", data.detail);
            }
        } catch (error) {
            setCreateError('Network error. Is the backend server running?');
                    ...
                    <>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'center' }}>
                            ...
                            >
                                ...
                                >
                                    {creating ? <><Loader size={18} style={{ animation: 'spin 1s linear infinite' }} /> Generating...</> : <><Plus size={18} /> Start Session</>}
                                </button>
                                ...
                                {sessionCode && (
                                    <div style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                                            ...
                                            <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#FFC107', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Session Created</span>
                                        </div>
                                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', fontWeight: 600 }}>Share this code with the client:</div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                                            ...
                                            >
                                                <Hash size={20} color="#FFC107" />
                                            </button>
                                        </div>
                                        <div style={{ display: 'flex', gap: '0.75rem' }}>
                                            <button
                                                onClick={() => navigate(`/tenant/${sessionCode}`)}
                                                style={{
                                                    flex: 1,
                                        ...
                                        <div key={i} style={{ padding: '1rem', borderRadius: '1rem', background: 'var(--input-bg)', border: '1px solid var(--nav-border)', marginBottom: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <div style={{ display: 'flex', flex: 1, justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                    <button
                                                        onClick={() => navigate(`/tenant/${s.session_id}`)}
                                                        style={{
